...
internal class describe_ReactiveSystem : nspec
{
    private readonly IMatcher<TestEntity> _matcherAB = Matcher<TestEntity>.AllOf(CID.ComponentA, CID.ComponentB);

    private MyTestContext _context;

    private static void assertEntities(IReactiveSystemSpy system, TestEntity entity, int didExecute = 1)
    {
        if (entity == null)
        {
    ...
    private TestEntity createEntityAB()
    {
    ...
    private TestEntity createEntityAC()
    {
        return _context.CreateEntity()
            .AddComponentA()
            .AddComponentC();
    ...
    private TestEntity createEntityABC()
    {
        return _context.CreateEntity()
            .AddComponentA()
            .AddComponentB()
    ...
    private void when_created()
    {
        ...
        before = () => { _context = new MyTestContext(); };

        context["OnEntityAdded"] = () =>
        {
            before = () => { system = new ReactiveSystemSpy(_context.CreateCollector<TestEntity>(_matcherAB)); };
            ...
            it["can ToString"] = () => { system.ToString().should_be("ReactiveSystem(ReactiveSystemSpy)"); };
        };

        context["OnEntityRemoved"] = () =>
        {
            before = () => { system = new ReactiveSystemSpy(_context.CreateCollector(_matcherAB.Removed())); };
        ...
        {
            before = () => { system = new ReactiveSystemSpy(_context.CreateCollector(_matcherAB.AddedOrRemoved())); };
