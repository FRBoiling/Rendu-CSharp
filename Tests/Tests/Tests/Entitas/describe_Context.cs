...
internal class describe_Context : nspec
{
    private void when_created()
    {
        IContext<TestEntity> ctx = null;

        before = () => { ctx = new MyTestContext(); };
        ...
        it["starts with given creationIndex"] = () => { new MyTestContext(CID.TotalComponents, 42, null).CreateEntity().creationIndex.should_be(42); };

        it["has no entities when no entities were created"] = () => { ctx.GetEntities().should_be_empty(); };

        it["gets total entity count"] = () => { ctx.count.should_be(0); };
        ...
        it["has default ContextInfo"] = () =>
        {
            ...
            for (var i = 0; i < ctx.contextInfo.componentNames.Length; i++) ctx.contextInfo.componentNames[i].should_be("Index " + i);
        };
        ...
        it["can ToString"] = () => { ctx.ToString().should_be("Unnamed Context"); };
        ...
        context["when ContextInfo set"] = () =>
        {
            ...
            it["has custom ContextInfo"] = () => { ctx.contextInfo.should_be_same(contextInfo); };

            it["creates entity with same ContextInfo"] = () => { ctx.CreateEntity().contextInfo.should_be_same(contextInfo); };
        ...
        context["when entity created"] = () =>
        {
            ...
            it["gets total entity count"] = () => { ctx.count.should_be(1); };

            it["has entities that were created with CreateEntity()"] = () => { ctx.HasEntity(e).should_be_true(); };

            it["doesn't have entities that were not created with CreateEntity()"] = () => { ctx.HasEntity(this.CreateEntity()).should_be_false(); };
            ...
            {
                var didDestroy = 0;
                ctx.OnEntityWillBeDestroyed += delegate { didDestroy += 1; };
                e.Destroy();
            ...
            it["ensures same deterministic order when getting entities after destroying all entities"] = () =>
            {
                ...
                for (var i = 0; i < numEntities; i++) ctx.CreateEntity();

                var order1 = new int[numEntities];
                var entities1 = ctx.GetEntities();
                for (var i = 0; i < numEntities; i++) order1[i] = entities1[i].creationIndex;
                ...
                for (var i = 0; i < numEntities; i++) order2[i] = entities2[i].creationIndex;
        ...
        context["groups"] = () =>
        {
            ...
            context["when entities created"] = () =>
            {
                ...
                IMatcher<TestEntity> matcherAB = Matcher<TestEntity>.AllOf(CID.ComponentA, CID.ComponentB);
                ...
                it["gets cached group"] = () => { ctx.GetGroup(matcherAB).should_be_same(ctx.GetGroup(matcherAB)); };
                ...
                context["event timing"] = () =>
                {
                    ...
                    it["dispatches group.OnEntityAdded events after all groups are updated"] = () =>
                    {
                        ...
                        groupA.OnEntityAdded += delegate { groupB.count.should_be(1); };
                    ...
                    it["dispatches group.OnEntityRemoved events after all groups are updated"] = () =>
                    {
                        ...
                        groupB.OnEntityRemoved += delegate { groupAB.count.should_be(0); };
        ...
        context["EntityIndex"] = () =>
        {
            it["throws when EntityIndex for key doesn't exist"] = expect<ContextEntityIndexDoesNotExistException>(() => { ctx.GetEntityIndex("unknown"); });
        ...
        context["reset"] = () =>
        {
            ...
            context["component pools"] = () =>
            {
                ...
                it["only clears existing component pool"] = () => { ctx.ClearComponentPool(CID.ComponentC); };
            };
        };
        ...
        context["EntitasCache"] = () =>
        {
            it["pops new list from list pool"] = () =>
            {
                ...
                groupAB.OnEntityAdded += (g, entity, index, component) => { didExecute += 1; };

                groupABC.OnEntityAdded += (g, entity, index, component) => { didExecute += 1; };
