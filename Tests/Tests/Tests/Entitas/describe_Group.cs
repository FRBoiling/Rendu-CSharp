...
internal class describe_Group : nspec
{
    private IGroup<TestEntity> _groupA;

    private void assertContains(params TestEntity[] expectedEntities)
    {
    ...
    private void assertContainsNot(TestEntity entity)
    {
    ...
    private void when_created()
    {
        ...
        {
            it["doesn't have entities which haven't been added"] = () => { _groupA.GetEntities().should_be_empty(); };
            ...
            it["doesn't contain entity"] = () => { _groupA.ContainsEntity(eA1).should_be_false(); };
        };

        context["when entity is matching"] = () =>
        {
            before = () => { handleSilently(eA1); };

            it["adds matching entity"] = () => { assertContains(eA1); };
            ...
            it["returns enumerable"] = () => { _groupA.AsEnumerable().Single().should_be_same(eA1); };
        ...
        it["gets null when single entity does not exist"] = () => { _groupA.GetSingleEntity().should_be_null(); };
        ...
        context["internal caching"] = () =>
        {
            context["GetEntities()"] = () =>
            {
                ...
                it["gets cached entities"] = () => { _groupA.GetEntities().should_be_same(cache); };
            ...
            context["SingleEntity()"] = () =>
            {
                ...
                it["gets cached singleEntities"] = () => { _groupA.GetSingleEntity().should_be_same(cache); };
    ...
    private void handleSilently(TestEntity entity)
    {
        _groupA.HandleEntitySilently(entity);
    }

    private void handle(TestEntity entity, int index, IComponent component)
    {
        _groupA.HandleEntity(entity, index, component);
    }

    private void handleAddEA(TestEntity entity)
    {
        handle(entity, CID.ComponentA, entity.GetComponentA());
    }

    private void handleAddEB(TestEntity entity)
    {
        handle(entity, CID.ComponentB, entity.GetComponentB());
    }

    private void handleRemoveEA(TestEntity entity, IComponent component)
    {
        handle(entity, CID.ComponentA, component);
    }

    private void updateEA(TestEntity entity, IComponent component)
    {
