...
namespace DesperateDevs.CodeGeneration.CodeGenerator
{
  public class CodeGenerator
  {
      ...
      bool trackHooks = true)
    {
      ...
      this._codeGenerators = ((IEnumerable<ICodeGenerator>) codeGenerators).OrderBy<ICodeGenerator, int>((Func<ICodeGenerator, int>) (i => i.priority)).ToArray<ICodeGenerator>();
      this._postProcessors = ((IEnumerable<IPostProcessor>) postProcessors).OrderBy<IPostProcessor, int>((Func<IPostProcessor, int>) (i => i.priority)).ToArray<IPostProcessor>();
    ...
    {
      return this.generate("[Dry Run] ", ((IEnumerable<IPreProcessor>) this._preProcessors).Where<IPreProcessor>((Func<IPreProcessor, bool>) (i => i.runInDryMode)).ToArray<IPreProcessor>(), ((IEnumerable<IDataProvider>) this._dataProviders).Where<IDataProvider>((Func<IDataProvider, bool>) (i => i.runInDryMode)).ToArray<IDataProvider>(), ((IEnumerable<ICodeGenerator>) this._codeGenerators).Where<ICodeGenerator>((Func<ICodeGenerator, bool>) (i => i.runInDryMode)).ToArray<ICodeGenerator>(), ((IEnumerable<IPostProcessor>) this._postProcessors).Where<IPostProcessor>((Func<IPostProcessor, bool>) (i => i.runInDryMode)).ToArray<IPostProcessor>());
    }
      ...
      IPostProcessor[] postProcessors)
    {
      ...
      foreach (ICachable cachable in ((IEnumerable<ICodeGenerationPlugin>) preProcessors).Concat<ICodeGenerationPlugin>((IEnumerable<ICodeGenerationPlugin>) dataProviders).Concat<ICodeGenerationPlugin>((IEnumerable<ICodeGenerationPlugin>) codeGenerators).Concat<ICodeGenerationPlugin>((IEnumerable<ICodeGenerationPlugin>) postProcessors).OfType<ICachable>())
