...
namespace DesperateDevs.CodeGeneration.CodeGenerator.Unity.Editor
{
  public static class UnityCodeGenerator
  {
    ...
    {
      return new Preferences(EditorPrefs.GetString("DesperateDevs.CodeGeneration.CodeGenerator.Unity.Editor.PropertiesPath", DesperateDevs.CodeGeneration.CodeGenerator.CodeGenerator.defaultPropertiesPath), Preferences.defaultUserPropertiesPath);
    }
    ...
    public static void Generate()
    {
      ...
      DesperateDevs.CodeGeneration.CodeGenerator.CodeGenerator codeGenerator = CodeGeneratorUtil.CodeGeneratorFromPreferences(UnityCodeGenerator.GetPreferences());
      ...
      Debug.Log((object) ("Generated " + (object) ((IEnumerable<CodeGenFile>) codeGenFileArray2).Select<CodeGenFile, string>((Func<CodeGenFile, string>) (file => file.fileName)).Distinct<string>().Count<string>() + " files (" + (object) ((IEnumerable<CodeGenFile>) codeGenFileArray1).Select<CodeGenFile, string>((Func<CodeGenFile, string>) (file => file.fileContent.ToUnixLineEndings())).Sum<string>((Func<string, int>) (content => content.Split(new char[1]
      {
        '\n'
      }, StringSplitOptions.RemoveEmptyEntries).Length)) + " sloc, " + (object) ((IEnumerable<CodeGenFile>) codeGenFileArray2).Select<CodeGenFile, string>((Func<CodeGenFile, string>) (file => file.fileContent.ToUnixLineEndings())).Sum<string>((Func<string, int>) (content => content.Split('\n').Length)) + " loc)"));
