...
namespace TcpLib.TcpSrc
{
    public partial class Tcp : ITcp
    {
        ...
        {
            Log.Debug("default on DefaultOnConnect function called,check it");
            return false;
        }

        static private bool DefaultOnAccept(bool ret)
        {
            Log.Debug("default on DefaultOnAccept function called,check it");
            return false;
        }
        ...
        {
            Log.Debug("default on DefaultOnDisconnect function called,check it");
            return false;
        }
        ...
        public bool Accept(ushort port)
        {
            ...
            {
                Log.Error("accept error:{0}", e.ToString());
                return false;
            }
            return true;
        }
        ...
        public bool Connect(string ip, ushort port)
        {
            ...
            else
            {
                ...
                {
                    Log.Error("connect error:{0}", e.ToString());
                    return false;
                }
            }
            return true;
        ...
        private void ConnectCallback(IAsyncResult ar)
        {
            ...
            try
            {
                ...
                Log.Debug("socket connected to {0}", socket.RemoteEndPoint.ToString());
                Recv(socket);
                OnConnect(true);
            }
            catch (Exception e)
        ...
        private void AcceptCallback(IAsyncResult ar)
        {
            ...
            {
                Log.Error("AcceptCallback error:{0}", e.ToString());
                OnAccept(false);
            }
            return;
        }
        ...
        private void RecvCallback(IAsyncResult ar)
        {
            ...
            if (handler == null)
            {
                Log.Error("Accept socket is null");
                return;
            }
            ...
            try
            {
                ...
                if (len <= 0)
                {
                    Log.Write("RecvCallback warning : len = {0} （len <= 0）", len);
                    Disconnect();
                    return;
                }
                Log.Write("Recv {0} bytes.", len);
                ...
                if (state.offset < 0)
                {
                    Log.Warn("RecvCallback warning : state.offset = {0} (state.offset < 0)", state.offset);
                    Disconnect();
                    return;
                }
                int size = 16384;
                ...
                if (size > 65535)
                {
                    Log.Warn("RecvCallback warning : size = {0} (size > 65535) ", size);
                    Disconnect();
                    return;
                }
