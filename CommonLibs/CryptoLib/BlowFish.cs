...
namespace CryptoLib
{
    public class BlowFish
    {
        ...
        public MemoryStream Decrypt_CBC(MemoryStream stream)
        {
            byte[] pt = stream.GetBuffer();
            ...
            IV = tempIV;
            byte[] tempAns = new byte[pt.Length - 8];
            for (int i = 8; i < pt.Length; i++)
            {
                tempAns[i - 8] = pt[i];
            }
            byte[] ans = Crypt_CBC(tempAns, true);
            MemoryStream trueStream = new MemoryStream(ans, 0, ans.Length, true, true);
            return trueStream;
        }


        public MemoryStream Decrypt_CBC(byte[] pt, bool up)
        {
        ...
        public MemoryStream Decrypt_CBC(MemoryStream stream, int offset, int size)
        {
            byte[] pt = stream.GetBuffer();
            byte[] tempIV = new byte[8];

            Buffer.BlockCopy(pt, offset + 8, tempIV, 0, 8);
            IV = tempIV;
            byte[] tempAns = new byte[size-8];

            Buffer.BlockCopy(pt, offset + 16, tempAns, 0, size-8);
            byte[] ans = Crypt_CBC(tempAns, true);
            MemoryStream trueStream = new MemoryStream(ans, 0, ans.Length, true, true);
            return trueStream;
        }

        public byte[] Decrypt_CBC_BTM(byte[] tmp)
        {
            byte[] pt = tmp;
            byte[] tempIV = new byte[8];
            for (int i = 0; i < 8; i++)
        ...
        public MemoryStream Encrypt_CBC_BTM(byte[] pt)
        {
            if (!IVSet)
                SetRandomIV();


            byte[] ans = Crypt_CBC(pt, false);

            int len = ans.Length + 8;
            byte[] realAns = new byte[len];
            Buffer.BlockCopy(InitVector, 0, realAns, 0, 8);
            Buffer.BlockCopy(ans, 0, realAns, 8, len - 8);
            MemoryStream trueStream = new MemoryStream(realAns, 0, realAns.Length, true, true);
            return trueStream;
        }

        public MemoryStream Encrypt_CBC(MemoryStream stream)
        {
            ...
            int len1 = (int)stream.Position;
            byte[] buffer = new byte[len1];
            stream.Seek(0, SeekOrigin.Begin);
            stream.Read(buffer, 0, len1);
            byte[] ans = Crypt_CBC(buffer, false);
