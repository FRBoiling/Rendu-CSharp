    ...
    public class CircularBuffer : Stream
    {
        ...
        public override long Length
        {
            get
            {
                ...
                //                if (c < 0)
                //                {
                //                    Log.Error("CircularBuffer count < 0: {0}, {1}, {2}".Fmt(this.bufferQueue.Count, this.LastIndex,
                //                        this.FirstIndex));
                //                }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                {
                    Array.Copy(First, FirstIndex, buffer, alreadyCopyCount + offset,
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            while (alreadyCopyCount < count)
            {
                ...
                {
                    Array.Copy(buffer, alreadyCopyCount + offset, lastBuffer, LastIndex,
                        ChunkSize - LastIndex);
                    alreadyCopyCount += ChunkSize - LastIndex;
                    LastIndex = ChunkSize;
                }
