...
namespace Rd.Migration.Migrations
{
    public class M0360_2 : IMigration
    {
        ...
        private string migrateToFilter(string fileContent)
        {
            ...
            if (!string.IsNullOrEmpty(ensureFilter)) ensureFilter = "(" + ensureFilter + ")";

            var excludeFilter = getFilter(excludeMatcher);
            if (!string.IsNullOrEmpty(excludeFilter)) excludeFilter = "!(" + excludeFilter + ")";

            var filter = "true";
            if (ensureFilter != null) filter = ensureFilter;
...
{1}
            ...
            if (!string.IsNullOrEmpty(setPoolsLogic)) construtorLogic.Add("        " + setPoolsLogic);

            if (!string.IsNullOrEmpty(setPoolLogic)) construtorLogic.Add("        " + setPoolLogic);
