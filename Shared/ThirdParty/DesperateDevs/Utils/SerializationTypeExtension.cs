  ...
  public static class SerializationTypeExtension
  {
    ...
    public static string ToCompilableString(this Type type)
    {
      ...
      if (type.IsGenericType)
        return type.FullName.Split('`')[0] + "<" + string.Join(", ", ((IEnumerable<Type>) type.GetGenericArguments()).Select<Type, string>((Func<Type, string>) (argType => argType.ToCompilableString())).ToArray<string>()) + ">";
      if (type.IsArray)
    ...
    {
      typeString = Regex.Replace(typeString, "(?<type>[^\\[]*)(?<rank>\\[,*\\])", (MatchEvaluator) (m => SerializationTypeExtension.generateTypeString(m.Groups["type"].Value) + m.Groups["rank"].Value));
      return typeString;
    }
  }
}